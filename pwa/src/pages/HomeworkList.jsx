...
import { FaBook, FaCalendarAlt, FaCheckCircle, FaClock, FaExclamationTriangle, FaSpinner, FaFileAlt, FaPlus } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchHomeworks = async () => {
      ...
      try {
        let url;
        if (isTeacher) {
          url = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000'}/api/homeworks/for-teacher/${teacherId}`;
        } else {
          url = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000'}/api/homeworks/for-parent/${parent_id}?child_id=${selectedChild}`;
        }
